cat << 'EOF' > calc_config.py
from math import *
import statistics as st
import sympy as sp

# ==========================================
# 1. OPERACIONES BÁSICAS Y UTILIDADES
# ==========================================
def sumar(a, b):
    return a + b
...
PI, E, G, MU_0 = pi, e, 9.80665, 4 * pi * 1e-7
...
# ==========================================
# 2. ESTADÍSTICA Y GEOMETRÍA VECTORIAL
# ==========================================
def media(*d):
    return st.mean(d)
...
def rec2pol(x, y):
    """Convierte coordenadas rectangulares a polares (r, deg)."""
    return sqrt(x**2 + y**2), degrees(atan2(y, x))

def pol2rec(r, deg):
    """Convierte coordenadas polares a rectangulares (x, y)."""
    return r * cos(radians(deg)), r * sin(radians(deg))

# ==========================================
# 3. FÍSICA Y ELECTROMAGNETISMO
# ==========================================
def fuerza_mag(q, v, B, deg=90):
    return abs(q) * v * B * sin(radians(deg))
...
def B_solenoide(I, N, L):
    return MU_0 * (N / L) * I

# ==========================================
# 4. CÁLCULO SIMBÓLICO Y MULTIVARIABLE (SymPy)
# ==========================================
...
def itriple(e, lx, ly, lz):
    """Calcula la integral triple int(int(int(e, dz), dy), dx)."""
    return sp.integrate(
        sp.sympify(e), (z, lz[0], lz[1]), (y, ly[0], ly[1]), (x, lx[0], lx[1])
    )

def masa_3d(densidad, lx, ly, lz):
    """Calcula la masa de un sólido 3D dada su función de densidad rho(x,y,z)."""
    return itriple(densidad, lx, ly, lz)

def centroide_3d(densidad, lx, ly, lz):
    """Calcula las coordenadas del centro de masa (x_bar, y_bar, z_bar) de un sólido."""
...
def _obtener_contexto_sympy():
    """Genera el diccionario de contexto extendido para evaluación de EDOs."""
    return {
...
def menu():
    while True:
        ...
        print("4. Física y Electromagnetismo")
        print("5. Derivadas e Integrales (SymPy)")
        ...
        if opcion == "1":
            ...
            print("\n--- Estadística ---")
            datos = list(map(float, input("Ingrese los datos separados por espacio: ").split()))
            if datos:
            ...
            print("\n--- Física ---")
            I = float(input("Corriente (I): "))
            r = float(input("Distancia (r): "))
            print(f"B en cable recto: {B_cable(I, r)} T")

        elif opcion == "5":
            print("\n--- Cálculo Simbólico ---")
            expr = input("Expresión en términos de x (ej. x**2 + sin(x)): ")
